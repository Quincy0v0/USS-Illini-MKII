...
/**
 * Clan class that renders details for the Clan pagess
 * @extends {Component}
 */
export default class ClanIndex extends Component {
    ...
    axios.get('https://api.worldofwarships.com/wows/clans/info/?application_id=' + application_id + '&clan_id=' + clan_id)
      .then((response) => {
        ...
        for (var i = 0; i < response.data.data[clan_id].members_ids.length / slice; i++) {
          let account_id_strings = '';
          let limit;
          if (i * slice + slice < response.data.data[clan_id].members_ids.length) {
            limit = i * slice + slice;
          } else {
            limit = response.data.data[clan_id].members_ids.length;
          }
  ...
  /**
   * Renders a listview of clan members
   */
  buildMembers() {
    let arr = [];
            ...
            <div key={row.nickname}>
              ...
              >
                <div style={{ margin: '1.5em' }}>
                  ...
                  <Popup
                    trigger={<NavLink
                      to={{ pathname: '/player', state: { account_id: row.account_id } }}>{row.nickname}</NavLink>}
                    header='Position'
          ...
          (
            <div key={row.nickname}>
              ...
              >
                <div style={{ margin: '1.5em' }}>
                  <Icon name='user circle' size='large' />
                  <NavLink to={{ pathname: '/player', state: { account_id: row.account_id } }}>
                    {row.nickname}
                  </NavLink>
                </div>
                <div
    ...
    return (
      <Container fluid>
        ...
        >
          <Header
            as='h4'
            onClick={() => this.setState({ playerTableVisible: !this.state.playerTableVisible })}
          >
