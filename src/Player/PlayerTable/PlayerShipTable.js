...
const nationOptions=[
  ...
  {key: 'europe', value: 'europe', image: 'https://wiki.gcdn.co/images/5/52/Wows_flag_Europe.png', text: 'Europe' }
];
...
class PlayerShipTableBody extends Component {
  ...
  componentDidMount() {
    axios.get("https://cors-anywhere.herokuapp.com/https://api.wows-numbers.com/personal/rating/expected/json/").then(res =>
      this.setState({standard: res.data.data})
    );
  }
  ...
  build(data,page,selectedName,selectedNation,selectedType,selectedTier){
    let rows = [];
    if(data){
      for (let i = page * perpage; i < data.length; i++){
        let row = data[i];
        if (i < page * perpage + perpage){
          let UIR = this.state.standard[row.ship_id] ? Math.round( 1000 * row.ave_damage_dealt/this.state.standard[row.ship_id].average_damage_dealt
            +300 * row.ave_frags/this.state.standard[row.ship_id].average_frags
            +700 * row.win_rate/this.state.standard[row.ship_id].win_rate ) : 0;
          let win_rate = Math.round(row.win_rate * 100);
          rows.push(
              (
              <Table.Row key={row.ship_id} id={"PlayerShipTable"+row.ship_id.toString()} onClick={(e)=>{this.props.handleselectedShipID(e.currentTarget.id)}}>
                ...
                <Table.Cell style={win_rate>60?{color: 'purple'}:UIR>50?{color: 'green'}:{color: 'red'}}>{win_rate+"%"}</Table.Cell>
                <Table.Cell style={row.survival_rate>0.5?{color: 'purple'}:row.survival_rate>0.4?{color: 'green'}:{color: 'red'}}>{row.survival_rate}</Table.Cell>
                <Table.Cell style={UIR>2000?{color: 'purple'}:UIR>1000?{color: 'green'}:{color: 'red'}}>{UIR}</Table.Cell>

                <Table.Cell >{row.max_xp}</Table.Cell>
                <Table.Cell >{row.max_frags_battle}</Table.Cell>
                <Table.Cell style={!this.state.standard[row.ship_id] ? null : row.max_damage_dealt>1.7*this.state.standard[row.ship_id].average_damage_dealt?{color: 'purple'}:row.max_damage_dealt>this.state.standard[row.ship_id].average_damage_dealt?{color: 'green'}:{color: 'red'}} >{row.max_damage_dealt}</Table.Cell>
                <Table.Cell >{row.max_planes_killed}</Table.Cell>

                <Table.Cell >{row.ave_xp}</Table.Cell>
                <Table.Cell >{row.ave_frags}</Table.Cell>
                <Table.Cell style={!this.state.standard[row.ship_id] ? null : row.ave_damage_dealt>1.7*this.state.standard[row.ship_id].average_damage_dealt?{color: 'purple'}:row.ave_damage_dealt>this.state.standard[row.ship_id].average_damage_dealt?{color: 'green'}:{color: 'red'}} >{row.ave_damage_dealt}</Table.Cell>
        ...
        <Table sortable selectable celled structured striped unstackable className="PlayerShipTable">
            <Table.Header className="PlayerShipTableHeader">
              <Table.Row key="header1">
                ...
                <Table.HeaderCell colSpan='2'>Nation</Table.HeaderCell>
                ...
                <Table.HeaderCell rowSpan='2'>UIR</Table.HeaderCell>
